...
export const Passport = () => {
  ...
  return (
    <div className="min-height-screen pb-20">
      {/* Header */}
      <motion.section
      ...
      >
        <div>
          <h1 className="text-5xl font-serif font-bold text-brown-primary mb-2">{product.name}</h1>
          <p className="text-2xl text-gold font-semibold mb-1">{product.umkm}</p>
          <p className="text-lg text-brown-light mb-4">📍 {product.village}</p>
          <div className="flex flex-wrap gap-3">
            {product.ethicalBadges.map((badge, idx) => (
              <motion.span
                key={idx}
                className="bg-gradient-to-r from-green-accent to-green-accent/60 text-white px-4 py-2 rounded-full font-semibold text-sm"
      ...
      {/* UMKM Story */}
      <motion.section
        className="max-w-6xl mx-auto px-8 py-8 bg-white rounded-2xl shadow-sm mb-8"
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
        transition={{ duration: 0.6 }}
      >
        <h2 className="text-3xl font-serif font-bold text-brown-primary mb-4">📖 The UMKM Story</h2>
        <p className="text-lg text-dark leading-relaxed">{product.umkmStory}</p>
      </motion.section>
      ...
      >
        <h2 className="text-3xl font-serif font-bold text-brown-primary mb-4">🌿 Cultural Value</h2>
        <p className="text-lg text-dark leading-relaxed">{product.culturalValue}</p>
      </motion.section>
      ...
      <motion.section
        ...
        transition={{ duration: 0.6 }}
      >
        <h2 className="text-3xl font-serif font-bold text-brown-primary mb-6">✋ Meet the Artisan</h2>
      ...
      {quizQuestions.length > 0 && (
        ...
        >
          <div className="mb-8">
            <h2 className="text-3xl font-serif font-bold text-brown-primary mb-2">📚 Test Your Knowledge</h2>
            <p className="text-lg text-brown-light">Learn more about this product's ethical journey!</p>
          </div>

          {!showQuiz ? (
            <motion.button
              className="w-full py-4 bg-gradient-to-r from-brown-primary to-gold text-white font-semibold rounded-lg hover:shadow-lg transition-shadow"
